...
M.exec = function(options)
    ...
    local function substitute_placeholders(input)
        ...
        if string.find(text, "%$buffers") then
            local buffer_context = get_context_from_open_buffers()
            text = string.gsub(text, "%$buffers", buffer_context)
        end
...
function get_context_from_open_buffers()
    local bufs = vim.api.nvim_list_bufs()

    local context = "The following files are open in my editor:\n"

    for _, buf in ipairs(bufs) do
        local name = vim.api.nvim_buf_get_name(buf)
        -- Check if the buffer has a name and is not a special buffer
